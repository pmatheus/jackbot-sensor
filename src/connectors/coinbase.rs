...
//!
//! This module provides the Coinbase implementation of the Exchange trait,
//! wrapping the jackbot-execution Coinbase client to provide a unified interface.
...
/// Coinbase connector implementing the Exchange trait
pub struct CoinbaseConnector {
    client: Arc<Mutex<Option<CoinbaseClient>>>,
...
impl CoinbaseConnector {
    ...
    pub fn new(
        ...
        // For Coinbase, we need a passphrase as well
    ...
    /// Process market data from Coinbase WebSocket
...
impl Exchange for CoinbaseConnector {
    async fn connect(&self) -> Result<Connection> {
        info!("Connecting to Coinbase exchange");
        ...
        let config = if let (Some(api_key), Some(api_secret), Some(api_passphrase)) = 
            ...
            CoinbaseConfig {
                ...
                ws_auth_payload: format!("{{\"api_key\":\"{}\"}}", api_key),
            }
        } else {
            // Use sandbox/demo credentials
            CoinbaseConfig {
                api_key: "demo".to_string(),
                api_secret: "demo".to_string(),
                api_passphrase: "demo".to_string(),
                sandbox: true,
                ws_auth_payload: "{\"api_key\":\"demo\"}".to_string(),
            }
        };
    ...
    async fn cancel_order(&self, id: OrderId) -> Result<()> {
        ...
        let request = OrderRequestCancel {
            key: OrderKey {
                exchange: ExchangeId::Coinbase,
                instrument: InstrumentNameExchange::new("BTC-USD".to_string()), // Instrument tracking - see INSTRUMENT_MAPPING_SPEC.md
                strategy: StrategyId::new("sensor".to_string()),
...
impl Clone for CoinbaseConnector {
    fn clone(&self) -> Self {
        Self {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_coinbase_connector_creation() {
        let connector = CoinbaseConnector::new(None, None, true);
        assert!(connector.is_ok());
    }

    #[tokio::test]
    async fn test_coinbase_connector_connect() {
        let connector = CoinbaseConnector::new(None, None, true).unwrap();
