...
mod tests {
    ...
    fn test_hyperliquid_futures_order_book_l2() {
        ...
        assert_eq!(
            ...
            Level::new(dec!(30000.0), dec!(1.0))
        );
        assert_eq!(
            canonical.asks().levels()[0],
            Level::new(dec!(30010.0), dec!(2.0))
        );
    }

    #[test]
    fn test_store_methods() {
        // Test proper deserialization with subscription_id generation
        let input = r#"{"coin":"BTC","bids":[["30000.0","1.0"]],"asks":[["30010.0","2.0"]]}"#;
        let book: HyperliquidFuturesOrderBookL2 = serde_json::from_str(input).unwrap();
        ...
        assert!(
            store
                .get_snapshot(ExchangeId::Hyperliquid, book.subscription_id.as_ref())
                .is_some()
        );

        let delta_book = HyperliquidFuturesOrderBookL2 {
            time: Utc::now(),
            ..book.clone()
        };
        delta_book.store_delta(&store);
        assert_eq!(
            store.delta_len(ExchangeId::Hyperliquid, book.subscription_id.as_ref()),
            1
        );
    }
}
