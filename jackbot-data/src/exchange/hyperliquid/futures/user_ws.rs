//! User WebSocket handling for Hyperliquid Futures.
pub use crate::exchange::user_ws_common::{user_stream, UserWsEvent as HyperliquidUserEvent};
...
mod tests {
    ...
    async fn test_user_stream_parse() {
        ...
        assert!(matches!(stream.next().await.unwrap(), HyperliquidUserEvent::Balance{..}));
        assert!(matches!(stream.next().await.unwrap(), HyperliquidUserEvent::Order{..}));
        assert!(matches!(stream.next().await.unwrap(), HyperliquidUserEvent::Position{..}));
    }
}
