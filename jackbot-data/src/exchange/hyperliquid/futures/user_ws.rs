...
pub use crate::exchange::user_ws_common::{UserWsEvent as HyperliquidUserEvent, user_stream};
...
mod tests {
    ...
    async fn test_user_stream_parse() {
        ...
        assert!(matches!(
            ...
            HyperliquidUserEvent::Balance { .. }
        ));
        assert!(matches!(
            stream.next().await.unwrap(),
            HyperliquidUserEvent::Order { .. }
        ));
        assert!(matches!(
            stream.next().await.unwrap(),
            HyperliquidUserEvent::Position { .. }
        ));
    }
}
