//! User WebSocket handling for Kucoin Spot.
pub use crate::exchange::user_ws_common::{UserWsEvent as KucoinUserEvent, user_stream};
...
mod tests {
    ...
    async fn test_user_stream_parse() {
        ...
        assert!(matches!(
            ...
            KucoinUserEvent::Balance { .. }
        ));
        assert!(matches!(
            stream.next().await.unwrap(),
            KucoinUserEvent::Order { .. }
        ));
        assert!(matches!(
            stream.next().await.unwrap(),
            KucoinUserEvent::Position { .. }
        ));
    }
}
