...
{
    fn from(
        (exchange_id, instrument, book): (ExchangeId, InstrumentKey, KucoinFuturesOrderBookL2),
    ) -> Self {
...
mod tests {
    ...
    fn test_kucoin_futures_order_book_l2() {
        let input =
            r#"{"symbol":"BTC-USDT","bids":[["30000.0","1.0"]],"asks":[["30010.0","2.0"]]}"#;
    ...
    fn test_store_methods() {
        ...
        assert!(
            store
                .get_snapshot_json(ExchangeId::Kucoin, "BTC-USDT")
                .is_some()
        );

        let delta_book = KucoinFuturesOrderBookL2 {
            time: Utc::now(),
            ..book
        };
