    ...
    InstrumentKey: Clone,
{
    ...
    async fn init<T>(
        __instrument_map: T,
        __initial_snapshots: &[Self::Output], 
        __sink_tx: tokio::sync::mpsc::UnboundedSender<jackbot_integration::protocol::websocket::Message>
    ...
    fn test_sequencer_validate_first_update() {
        ...
        for (index, test) in tests.into_iter().enumerate() {
            ...
            match (actual, test.expected) {
                (Ok(_actual), Ok(_expected)) => {
                    // Test passed if both are Ok, no need to compare () with ()
                }
                (Err(_), Err(_)) => {
                    // Test passed
                }
