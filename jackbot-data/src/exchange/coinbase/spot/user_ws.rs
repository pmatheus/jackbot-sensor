//! User WebSocket handling for Coinbase Spot.
pub use crate::exchange::user_ws_common::{user_stream, UserWsEvent as CoinbaseUserEvent};
...
mod tests {
    ...
    async fn test_user_stream_parse() {
        ...
        assert!(matches!(stream.next().await.unwrap(), CoinbaseUserEvent::Balance{..}));
        assert!(matches!(stream.next().await.unwrap(), CoinbaseUserEvent::Order{..}));
        assert!(matches!(stream.next().await.unwrap(), CoinbaseUserEvent::Position{..}));
    }
}
