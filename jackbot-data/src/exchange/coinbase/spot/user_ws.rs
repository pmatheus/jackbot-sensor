...
pub use crate::exchange::user_ws_common::{UserWsEvent as CoinbaseUserEvent, user_stream};
...
mod tests {
    ...
    async fn test_user_stream_parse() {
        ...
        assert!(matches!(
            ...
            CoinbaseUserEvent::Balance { .. }
        ));
        assert!(matches!(
            stream.next().await.unwrap(),
            CoinbaseUserEvent::Order { .. }
        ));
        assert!(matches!(
            stream.next().await.unwrap(),
            CoinbaseUserEvent::Position { .. }
        ));
    }
}
