//! User WebSocket handling for Kraken Futures.
pub use crate::exchange::user_ws_common::{user_stream, UserWsEvent as KrakenUserEvent};
...
mod tests {
    ...
    async fn test_user_stream_parse() {
        ...
        assert!(matches!(stream.next().await.unwrap(), KrakenUserEvent::Balance{..}));
        assert!(matches!(stream.next().await.unwrap(), KrakenUserEvent::Order{..}));
        assert!(matches!(stream.next().await.unwrap(), KrakenUserEvent::Position{..}));
    }
}
