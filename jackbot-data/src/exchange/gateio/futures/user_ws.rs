//! User WebSocket handling for Gateio Futures.
pub use crate::exchange::user_ws_common::{UserWsEvent as GateioUserEvent, user_stream};
...
mod tests {
    ...
    async fn test_user_stream_parse() {
        ...
        assert!(matches!(
            ...
            GateioUserEvent::Balance { .. }
        ));
        assert!(matches!(
            stream.next().await.unwrap(),
            GateioUserEvent::Order { .. }
        ));
        assert!(matches!(
            stream.next().await.unwrap(),
            GateioUserEvent::Position { .. }
        ));
    }
}
