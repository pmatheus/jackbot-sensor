//! User WebSocket handling for Okx Futures.
pub use crate::exchange::user_ws_common::{user_stream, UserWsEvent as OkxUserEvent};
...
mod tests {
    ...
    async fn test_user_stream_parse() {
        ...
        assert!(matches!(stream.next().await.unwrap(), OkxUserEvent::Balance{..}));
        assert!(matches!(stream.next().await.unwrap(), OkxUserEvent::Order{..}));
        assert!(matches!(stream.next().await.unwrap(), OkxUserEvent::Position{..}));
    }
}
