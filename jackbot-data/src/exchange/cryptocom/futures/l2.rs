...
{
    fn from(
        (exchange_id, instrument, book): (ExchangeId, InstrumentKey, CryptocomFuturesOrderBookL2),
    ) -> Self {
...
mod tests {
    ...
    fn test_store_methods() {
        ...
        assert!(
            store
                .get_snapshot_json(ExchangeId::Cryptocom, "BTC_USDT")
                .is_some()
        );

        let delta_book = CryptocomFuturesOrderBookL2 {
            time: Utc::now(),
            ..book
        };
