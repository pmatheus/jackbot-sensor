...
pub use crate::exchange::user_ws_common::{UserWsEvent as CryptocomUserEvent, user_stream};
...
mod tests {
    ...
    async fn test_user_stream_parse() {
        ...
        assert!(matches!(
            ...
            CryptocomUserEvent::Balance { .. }
        ));
        assert!(matches!(
            stream.next().await.unwrap(),
            CryptocomUserEvent::Order { .. }
        ));
        assert!(matches!(
            stream.next().await.unwrap(),
            CryptocomUserEvent::Position { .. }
        ));
    }
}
