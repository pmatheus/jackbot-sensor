//! User WebSocket handling for Cryptocom Futures.
pub use crate::exchange::user_ws_common::{user_stream, UserWsEvent as CryptocomUserEvent};
...
mod tests {
    ...
    async fn test_user_stream_parse() {
        ...
        assert!(matches!(stream.next().await.unwrap(), CryptocomUserEvent::Balance{..}));
        assert!(matches!(stream.next().await.unwrap(), CryptocomUserEvent::Order{..}));
        assert!(matches!(stream.next().await.unwrap(), CryptocomUserEvent::Position{..}));
    }
}
