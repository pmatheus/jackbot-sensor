//! User WebSocket handling for Bybit Futures.
pub use crate::exchange::user_ws_common::{user_stream, UserWsEvent as BybitUserEvent};
...
mod tests {
    ...
    async fn test_user_stream_parse() {
        ...
        assert!(matches!(stream.next().await.unwrap(), BybitUserEvent::Balance{..}));
        assert!(matches!(stream.next().await.unwrap(), BybitUserEvent::Order{..}));
        assert!(matches!(stream.next().await.unwrap(), BybitUserEvent::Position{..}));
    }
}
