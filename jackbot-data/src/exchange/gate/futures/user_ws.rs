//! User WebSocket handling for Gate Futures.
pub use crate::exchange::user_ws_common::{user_stream, UserWsEvent as GateUserEvent};
...
mod tests {
    ...
    async fn test_user_stream_parse() {
        ...
        assert!(matches!(stream.next().await.unwrap(), GateUserEvent::Balance{..}));
        assert!(matches!(stream.next().await.unwrap(), GateUserEvent::Order{..}));
        assert!(matches!(stream.next().await.unwrap(), GateUserEvent::Position{..}));
    }
}
