...
fn aggregates_books_by_weight() {
    ...
    // Best bid should be from book_b (higher price) with its weight
    assert_eq!(merged.bids().levels()[0].amount, dec!(1));
    // Best ask should be from book_a (lower price) with its weight
    assert_eq!(merged.asks().levels()[0].amount, dec!(2));
}
