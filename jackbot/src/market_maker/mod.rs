...
use jackbot_execution::market_making::{MarketMaker, Quote as MmQuote};
...
impl MarketMaker for InventorySkewQuoter {
    fn make_quote(&self, mid_price: Decimal, inventory_ratio: Decimal) -> MmQuote {
        let q = InventorySkewQuoter::quote(self, mid_price, inventory_ratio);
        MmQuote::new(q.bid_price, q.ask_price)
    }
}
...
impl MarketMaker for RiskControls {
    fn allow_trade(&self, inventory_ratio: Decimal) -> bool {
        RiskControls::check_inventory(self, inventory_ratio)
    }
}
...
impl MarketMaker for PerformanceTracker {
    fn record_trade(&mut self, pnl: Decimal) {
        PerformanceTracker::record_trade(self, pnl);
    }
}
...
pub fn print_quotes(quote: Quote) {
    ...
    table.add_row(Row::new(vec![
        Cell::new("Bid"),
        Cell::new(&quote.bid_price.to_string()),
    ]));
    table.add_row(Row::new(vec![
        Cell::new("Ask"),
        Cell::new(&quote.ask_price.to_string()),
    ]));
    table.printstd();
}
