    ...
    Event: Debug + TimeExchange,
{
    ...
    fn process(&mut self, event: &Event) -> Self::Audit {
        ...
        // Input event is more recent - update the clock
        if time_event_exchange >= lock.time_exchange_last {
            self.update_clock_time(&mut lock, event, time_event_exchange);
            return;
        }

        // Input event is older - handle out-of-order event
        self.handle_out_of_order_event(&lock, event, time_event_exchange);
    }
}

impl HistoricalClock {
    fn update_clock_time<Event: Debug>(
        &self,
        lock: &mut parking_lot::RwLockWriteGuard<'_, HistoricalClockInner>,
        event: &Event,
        time_event_exchange: DateTime<Utc>,
    ) {
        debug!(
            ?event,
    ...
    fn handle_out_of_order_event<Event: Debug>(
        &self,
        lock: &parking_lot::RwLockWriteGuard<'_, HistoricalClockInner>,
        event: &Event,
        time_event_exchange: DateTime<Utc>,
    ) {
        ...
        match time_diff_secs {
            0 => {
                let context = log_context();
                debug!(
                    event = ?context.0,
                    time_exchange_last_current = ?context.1,
                    time_update = ?context.2,
                    time_diff_secs = context.3,
                    message = context.4
                );
            }
            1..=29 => {
...
mod tests {
    ...
    fn test_historical_clock_time_delta_calculation() {
        ...
        assert!(
            delta_ms >= 90 && delta_ms <= 150,
            "Historical clock time delta outside expected range: {}ms",
            delta_ms
        );
    }
}
