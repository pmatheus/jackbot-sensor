use Jackbot::smart_trade::{
    MultiLevelStop, MultiLevelTakeProfit, ProfitTarget, SmartTradeSignal, TrailingStop,
    TrailingTakeProfit,
};
...
fn test_trailing_take_profit() {
    ...
    assert_eq!(
        tp.update(dec!(99)),
        Some(SmartTradeSignal::TakeProfit(dec!(99)))
    );
...
fn test_profit_target() {
    ...
    assert_eq!(
        pt.update(dec!(150)),
        Some(SmartTradeSignal::TakeProfit(dec!(150)))
    );
...
fn test_trailing_stop() {
    ...
    assert_eq!(
        ts.update(dec!(103)),
        Some(SmartTradeSignal::StopLoss(dec!(103)))
    );
...
fn test_multi_level_stop() {
    ...
    assert_eq!(
        ms.update(dec!(89)),
        Some(SmartTradeSignal::StopLevel(0, dec!(89)))
    );
    assert_eq!(
        ms.update(dec!(79)),
        Some(SmartTradeSignal::StopLevel(1, dec!(79)))
    );
...
fn test_multi_level_take_profit() {
    ...
    assert_eq!(
        mtp.update(dec!(120)),
        Some(SmartTradeSignal::TakeProfit(dec!(120)))
    );
